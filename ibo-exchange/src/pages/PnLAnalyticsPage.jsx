...
const fieldShell =
  'mt-1.5 w-full rounded-xl bg-[color:var(--ibo-elevated)] border border-[color:var(--ibo-border-solid)] px-3 py-2.5 text-sm font-semibold text-[color:var(--ibo-ink)] focus:outline-none focus:border-[rgba(254, 157, 85,0.55)] transition-colors';
const labelShell =
...
export default function PnLAnalyticsPage({ accountMode = false } = {}) {
  ...
  return (
    <div className={`${accountMode ? 'font-ui pb-8' : 'ibo-page font-ui pb-16'}`}>
      <div className={accountMode ? 'w-full min-w-0' : 'w-full px-4 sm:px-5 md:px-6 lg:px-8 xl:px-10 2xl:px-12 pt-6 sm:pt-8'}>

        {/* Hero — hidden when embedded in Delta account shell */}
        {!accountMode ? (
        <div
          className="relative overflow-hidden rounded-2xl border border-[color:var(--ibo-border-solid)] mb-6 p-5 sm:p-7"
          style={{
            background:
              'linear-gradient(135deg, var(--ibo-card) 0%, color-mix(in srgb, var(--ibo-card) 92%, #FE9D55) 55%, var(--ibo-card) 100%)',
          }}
        >
          <div
            className="pointer-events-none absolute right-0 top-0 h-full w-1/3 opacity-25"
            style={{ background: 'radial-gradient(ellipse at right center, #FE9D55, transparent 70%)' }}
          />
          <div className="relative flex flex-wrap items-center gap-3 sm:gap-4">
            <Link
              to="/trade/IBOUSDT"
              className="inline-flex items-center gap-2 text-[color:var(--ibo-muted)] hover:text-[#FE9D55] text-sm font-semibold transition-colors"
            >
            ...
            <button
              ...
              onClick={refreshAll}
              disabled={loading}
              className="ml-auto inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-[color:var(--ibo-border-solid)] text-sm font-bold text-[color:var(--ibo-ink)] hover:border-[rgba(254, 157, 85,0.4)] hover:bg-[color:var(--ibo-hover)] transition-colors disabled:opacity-40"
            >
          ...
          <div className="flex justify-end mb-4">
            <button
              ...
              onClick={refreshAll}
              disabled={loading}
              className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border border-[color:var(--ibo-border-solid)] text-xs font-bold text-[color:var(--ibo-ink)] hover:border-[#fe6c02]/45 transition-colors disabled:opacity-40"
            >
        ...
        <div className={`${cardShell} p-5 mb-8`}>
          <div className="flex items-center gap-2 text-[#FE9D55] font-bold text-sm mb-4">
          ...
          <button
            ...
            className="mt-4 text-xs font-bold text-[#FE9D55] hover:underline"
          >
            Reset all filters
          </button>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 mb-10">
          ...
          >
            <p className="text-2xl font-extrabold font-mono text-[#FE9D55] tabular-nums">
        ...
        <section className="mb-12">
          ...
          <p className="text-[color:var(--ibo-muted)] text-sm mb-3 max-w-3xl">
            Spot holdings with average cost and mark P&amp;L. Use <span className="text-[#FE9D55] font-semibold">Close</span> to
          ...
          <div className={`${cardShell} overflow-hidden`}>
            {loading && positions.length === 0 ? (
              <div className="flex items-center justify-center py-16 gap-2 text-[color:var(--ibo-muted)]">
                <RefreshCw className="animate-spin text-[#FE9D55]" size={20} /> Loading…
              </div>
          ...
          <p className="text-[color:var(--ibo-muted)] text-xs mt-3">
            You can also sell from <Link to="/wallet" className="text-[#FE9D55] font-semibold hover:underline">Wallet</Link> (Sell opens the terminal on the sell tab) or from <Link to="/trade/IBOUSDT" className="text-[#FE9D55] font-semibold hover:underline">Trade</Link> → Assets.
          </p>
        </section>
        ...
        <section>
          ...
          <p className="text-[color:var(--ibo-muted)] text-sm mb-3">
            Each row is one execution (fill). <span className="text-[#FE9D55] font-semibold">Click a row</span> for IDs, fees, notional, and realized P&amp;L.
          </p>
          <div className={`${cardShell} overflow-hidden`}>
                    ...
                    {filteredTrades.map(t => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <span className="text-xs font-extrabold uppercase px-2 py-1 rounded-md bg-[rgba(254, 157, 85,0.12)] text-[#FE9D55]">
