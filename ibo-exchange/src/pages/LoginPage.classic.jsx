/**
 * PARKED — Classic IBO login UI (temporarily disabled).
...
const PERKS = [
  { icon: TrendingUp, title: 'Deep liquidity', desc: 'Spot USDT pairs and IBO markets in one terminal' },
    ...
    <div className="ibo-page font-ui flex flex-col lg:flex-row min-h-[100dvh] lg:h-[100dvh] lg:max-h-[100dvh] lg:overflow-hidden">
      ...
      <aside className="hidden lg:flex flex-col w-[min(44%,28rem)] xl:w-[30rem] shrink-0 relative overflow-hidden
        ...
        <div
          ...
          style={{
            backgroundImage:
              'linear-gradient(#0EA4AB 1px,transparent 1px),linear-gradient(90deg,#0EA4AB 1px,transparent 1px)',
            backgroundSize: '44px 44px',
          }}
        />

        <div className="relative z-10 flex flex-col gap-7 h-full min-h-0">
          <Link to="/" aria-label="IBO Exchange home" className="inline-block w-fit hover:opacity-90 transition-opacity shrink-0">
            <img
              src={LOGO}
              alt="IBO Exchange"
          ...
          <div className="space-y-2.5 shrink-0">
            {PERKS.map(({ icon: Icon, title, desc }) => (
              ...
              >
                <Icon size={15} className="text-[#5BB8FF] mt-0.5 flex-shrink-0" />
          ...
          <div className="w-full max-w-[22.5rem] my-auto mx-auto lg:mx-0 lg:ml-[max(0px,min(3rem,8%))]">
            <header className="mb-7">
              ...
              <p className="mt-2 text-sm text-[color:var(--ibo-muted)] leading-snug">
                Access your IBO Exchange account
              </p>
            </header>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
              <div>
                ...
                <div
                  className={`flex items-center bg-[color:var(--ibo-card)] border rounded-xl px-3.5 h-11 focus-within:border-[rgba(91,184,255,0.55)] transition-colors group ${
                    showFieldError('email') ? 'border-red-500/50' : 'border-[color:var(--ibo-border-solid)]'
                  }`}
                >
                  <Mail size={15} className="text-[color:var(--ibo-muted)] mr-2.5 group-focus-within:text-[#5BB8FF] transition-colors" />
                  <input
                    type="email"
                    value={email}
              ...
              <div>
                <div className="flex items-center justify-between gap-3 mb-1.5">
                  ...
                  <Link to="/forgot-password" className="text-xs text-[#5BB8FF] hover:underline font-medium shrink-0">
                    Forgot password?
                  </Link>
                </div>
                <div
                ...
                >
                  <Lock size={15} className="text-[color:var(--ibo-muted)] mr-2.5 group-focus-within:text-[#5BB8FF] transition-colors" />
                  <input
              ...
              <button
                type="submit"
                disabled={loading}
                className="mt-1 w-full flex items-center justify-center gap-2 bg-logo-gradient text-[#050a1a]
            ...
            <p className="text-center text-sm text-[color:var(--ibo-muted)] mt-6">
              ...
              <Link to="/register" className="text-[#5BB8FF] font-bold hover:underline">
                Create account
              </Link>
            </p>
