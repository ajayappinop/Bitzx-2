...
const BENEFITS = [
  ...
  { icon: Zap,         color: '#FE6C02', bg: 'rgba(254, 108, 2,0.1)', border: 'rgba(254, 108, 2,0.25)', title: 'Higher Limits',      desc: 'Post higher-volume ads and transact larger amounts per order.' },
  { icon: TrendingUp,  color: '#FE6C02', bg: 'rgba(167,139,250,0.1)',border: 'rgba(254, 108, 2,0.25)',title: 'Priority Listing',   desc: 'Your ads appear at the top of search results, driving more trade flow.' },
  ...
  { icon: Award,       color: '#FE6C02', bg: 'rgba(254, 108, 2,0.1)', border: 'rgba(254, 108, 2,0.25)', title: 'Lower Fees',         desc: 'Enjoy reduced platform fees on all P2P trades as a verified merchant.' },
  { icon: Star,        color: '#FE9D55', bg: 'rgba(254, 108, 2,0.08)',border: 'rgba(254, 108, 2,0.2)',  title: 'Reputation Score',   desc: 'Dedicated merchant leaderboard visibility to attract more counter-parties.' },
];
const REQUIREMENTS = [
  ...
  return (
    <div className="ibo-page font-ui">
      <div className="w-full px-4 sm:px-5 md:px-6 lg:px-8 xl:px-10 2xl:px-12 py-6 sm:py-8 pb-16">
        ...
        {/* ── Hero ─────────────────────────────────────────────────── */}
        <div className="relative overflow-hidden rounded-xl border border-[color:var(--ibo-border-solid)] p-8 sm:p-10 mb-8 text-center"
          style={{ background: 'radial-gradient(ellipse 70% 120% at 100% 0%, rgba(254, 108, 2, 0.12), transparent 55%), var(--ibo-bg)' }}>
          <div className="pointer-events-none absolute inset-0"
            style={{ background: 'radial-gradient(ellipse 60% 50% at 50% 0%,rgba(254, 108, 2,0.15),transparent)' }} />
          ...
          <div className="relative mx-auto max-w-xl">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl mb-5 mx-auto"
              style={{ background: 'rgba(254, 108, 2,0.12)', border: '2px solid rgba(254, 108, 2,0.25)', boxShadow: '0 0 30px rgba(254, 108, 2,0.15)' }}>
              <ShieldCheck size={28} className="text-[#FE9D55]" />
            </div>
        ...
        {status?.is_merchant && (
          <div className="flex items-center gap-4 rounded-2xl p-5 mb-7"
            style={{ background: 'rgba(254, 108, 2,0.07)', border: '1px solid rgba(254, 108, 2,0.2)', boxShadow: '0 0 30px rgba(254, 108, 2,0.05)' }}>
            <div className="w-12 h-12 rounded-xl flex items-center justify-center shrink-0"
              style={{ background: 'rgba(254, 108, 2,0.12)', border: '1px solid rgba(254, 108, 2,0.25)' }}>
              <ShieldCheck size={22} className="text-[#FE9D55]" />
            </div>
            <div>
              <p className="text-[#FE9D55] font-extrabold text-base">Verified Merchant</p>
        ...
        {status?.merchant_status === 'pending' && (
          <div className="flex items-center gap-4 rounded-2xl p-5 mb-7"
            style={{ background: 'rgba(254, 157, 85,0.08)', border: '1px solid rgba(254, 157, 85,0.22)' }}>
            <Clock size={20} className="text-[#FE9D55] shrink-0" />
            <div>
              <p className="text-[#FE9D55] font-bold text-sm">Application Under Review</p>
        ...
        <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4 mb-10">
          {BENEFITS.map(({ icon: Icon, color, bg, border, title, desc }) => (
            <div key={title} className="ibo-hover-lift ibo-hover-glow rounded-xl border border-[color:var(--ibo-border-solid)] bg-transparent p-5 space-y-3">
        ...
        <div className="grid lg:grid-cols-[1fr,420px] gap-6">
          ...
          {/* Requirements */}
          <div className="rounded-xl border border-[color:var(--ibo-border-solid)] bg-transparent p-6">
            <h3 className="text-white font-bold text-base mb-5 flex items-center gap-2">
              <Lock size={15} className="text-[#FE9D55]" /> Eligibility Requirements
            </h3>
            ...
            <div className="mt-6 rounded-xl p-4"
              style={{ background: 'rgba(254, 108, 2,0.06)', border: '1px solid rgba(254, 108, 2,0.15)' }}>
          ...
          <div className="rounded-2xl p-6 space-y-5"
            ...
            {status?.is_merchant ? (
              <div className="text-center py-4">
                <ShieldCheck size={40} className="mx-auto mb-3 text-[#FE9D55]" />
              ...
              <>
                ...
                <div className="space-y-1.5">
                  ...
                  <select value={form.experience} onChange={(e) => setForm((f) => ({ ...f, experience: e.target.value }))}
                    className="w-full rounded-xl bg-transparent border border-[color:var(--ibo-border-solid)] px-3.5 py-2.5 text-[color:var(--ibo-ink)] text-sm focus:outline-none focus:border-[rgba(254, 157, 85,0.55)] transition-colors">
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea rows={3} value={form.why}
                    ...
                    className="w-full rounded-xl bg-transparent border border-[color:var(--ibo-border-solid)] px-3.5 py-2.5 text-[color:var(--ibo-ink)] text-sm placeholder:text-[color:var(--ibo-muted)] resize-none focus:outline-none focus:border-[rgba(254, 157, 85,0.55)] transition-colors"
                  />
                </div>
                {error && (
                ...
                <button onClick={submit} disabled={busy}
                  className="ibo-hover-scale w-full py-3.5 rounded-xl text-sm font-extrabold inline-flex items-center justify-center gap-2 disabled:opacity-40 transition-all text-[#101013]"
                  style={{ background: 'linear-gradient(135deg,#FE6C02,#FE9D55)', boxShadow: '0 4px 24px rgba(254, 108, 2,0.3)' }}>
