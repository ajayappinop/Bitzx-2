...
const ST_BADGE = {
  in_progress: { label: 'Awaiting Payment', cls: 'border-gold/30 bg-[rgba(254, 157, 85,0.1)] text-gold' },
  paid_marked: { label: 'Awaiting Release', cls: 'border-[#FE6C02]/30 bg-[#FE6C02]/10 text-[#FE9D55]' },
...
export default function P2POrderDetailPage() {
  ...
  if (loading) return (
    <div className="ibo-page flex items-center justify-center">
      <Loader2 size={20} className="animate-spin text-[#FE6C02]" />
    </div>
  );
  if (!order) return (
    ...
    <div className="ibo-page font-ui">
      <div className="w-full px-4 sm:px-5 md:px-6 lg:px-8 xl:px-10 2xl:px-12 py-6 sm:py-8 pb-16">
        ...
        <div className="flex items-center gap-3 mb-6 flex-wrap">
          <Link to="/p2p/orders"
            className="ibo-hover-border flex items-center justify-center w-9 h-9 rounded-xl border border-[color:var(--ibo-border-solid)] bg-transparent text-white/60 hover:text-white transition-colors">
            <ArrowLeft size={16} />
          </Link>
          <div>
            <h1 className="text-white font-bold text-xl tracking-tight">Order <span className="font-mono text-[#FE9D55]">{order.order_id}</span></h1>
        ...
        {/* ── Progress stepper ─────────────────────────────────────── */}
        {!isDone && (
          <div className="rounded-xl border border-[color:var(--ibo-border-solid)] bg-transparent p-5 mb-6">
                ...
                return (
                  <div key={s} className="flex items-center flex-1 last:flex-none">
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-extrabold border-2 shrink-0 transition-all ${
                      ...
                      : cur ? 'border-[#FE6C02] bg-transparent text-[#FE9D55]'
            ...
            <div className="flex justify-between mt-3">
              {STEPS.map((s, i) => (
                <p key={s} className={`text-xs font-semibold ${
                  step > i + 1 ? 'text-green-400' : step === i + 1 ? 'text-[#FE9D55]' : 'text-white/25'
                }`}>{s}</p>
              ))}
            </div>
          </div>
        ...
        <div className="grid lg:grid-cols-[1fr,360px] gap-6">
          ...
          {/* ── Chat panel ───────────────────────────────────────── */}
          <div className="flex flex-col rounded-xl border border-[color:var(--ibo-border-solid)] bg-transparent" style={{ minHeight: 520 }}>

            {/* Chat header */}
            <div className="px-5 py-4 border-b border-[color:var(--ibo-border-solid)] flex items-center gap-3">
              <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: 'rgba(254, 108, 2,0.12)', border: '1px solid rgba(254, 108, 2,0.2)' }}>
                <MessageCircle size={13} className="text-[#FE9D55]" />
              </div>
              ...
              {counterparty && (
                <div className="ml-auto flex items-center gap-2 text-xs text-white/45">
                  <div className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold text-[#FE9D55]"
                    style={{ background: 'rgba(254, 108, 2,0.15)', border: '1px solid rgba(254, 108, 2,0.2)' }}>
                    {(counterparty.nickname || 'T')[0]}
                  </div>
                  <span>{counterparty.nickname}</span>
                  {counterparty.is_merchant && <ShieldCheck size={11} className="text-[#FE9D55]" />}
                </div>
              )}
            </div>
                ...
                return (
                  <div key={i} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[78%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
                      ...
                      style={isMe
                        ? { background: 'rgba(254, 108, 2,0.18)', border: '1px solid rgba(254, 108, 2,0.2)' }
            ...
            {!isDone && (
              <form onSubmit={sendMsg} className="px-4 py-3 border-t border-[color:var(--ibo-border-solid)] flex items-center gap-2">
                <button type="button" onClick={() => fileRef.current?.click()}
                  className="flex items-center justify-center w-9 h-9 rounded-xl text-white/40 hover:text-[#FE9D55] transition-colors"
                ...
                <input value={msgText} onChange={(e) => setMsgText(e.target.value)}
                  ...
                  className="flex-1 rounded-xl bg-transparent border border-[color:var(--ibo-border-solid)] px-3.5 py-2 text-sm text-[color:var(--ibo-ink)] placeholder:text-[color:var(--ibo-muted)] focus:outline-none focus:border-[rgba(254, 157, 85,0.55)] transition-colors"
                />
                <button type="submit" disabled={ab('upload') || (!msgText.trim() && !file)}
                  ...
                  style={{ background: 'linear-gradient(135deg,#FE6C02,#FE9D55)', boxShadow: '0 2px 12px rgba(254, 108, 2,0.3)' }}>
                  {ab('upload') ? <Loader2 size={14} className="animate-spin text-white" /> : <Send size={14} className="text-[#101013]" />}
                </button>
              </form>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            {/* Order summary */}
            <div className="rounded-xl border border-[color:var(--ibo-border-solid)] bg-transparent p-5 space-y-2.5">
            ...
            {canMarkPaid && (
              <div className="rounded-2xl p-5 space-y-3"
                style={{ background: 'rgba(0, 168, 118,0.06)', border: '1px solid rgba(0, 168, 118,0.2)' }}>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl bg-transparent border border-[color:var(--ibo-border-solid)] px-3.5 py-2.5 text-[color:var(--ibo-ink)] text-sm placeholder:text-[color:var(--ibo-muted)] focus:outline-none focus:border-[rgba(254, 157, 85,0.55)] transition-colors"
                  />
                </div>
            ...
            {canRelease && (
              <div className="rounded-2xl p-5 space-y-3"
                style={{ background: 'rgba(254, 108, 2,0.06)', border: '1px solid rgba(254, 108, 2,0.2)' }}>
                <div className="flex items-center gap-2 text-[#FE9D55] font-bold text-sm">
                ...
                <input type="text" inputMode="numeric" maxLength={6}
                  ...
                  className="w-full rounded-xl bg-transparent border border-[color:var(--ibo-border-solid)] px-3.5 py-2.5 text-white text-center tracking-widest font-mono text-base focus:outline-none focus:border-[rgba(254, 157, 85,0.55)]"
                />
                <button onClick={release} disabled={ab('release') || totpCode.length < 6}
                  className="ibo-hover-scale w-full py-3 rounded-xl text-sm font-bold transition-all disabled:opacity-40 inline-flex items-center justify-center gap-2"
                  style={{ background: 'rgba(254, 108, 2,0.15)', border: '1px solid rgba(254, 108, 2,0.3)', color: '#FE9D55' }}>
            ...
            {isBuyer && order.status === 'paid_marked' && (
              <div className="flex items-start gap-3 rounded-2xl p-4"
                style={{ background: 'rgba(254, 108, 2,0.05)', border: '1px solid rgba(254, 108, 2,0.15)' }}>
                <Info size={13} className="text-[#FE9D55] shrink-0 mt-0.5" />
                <p className="text-xs text-white/65 leading-relaxed">
          ...
          <P2PModal.Body>
            ...
            <div className="space-y-1.5">
              ...
              <select value={disputeReason} onChange={(e) => setDisputeReason(e.target.value)}
                className="w-full rounded-xl bg-transparent border border-[color:var(--ibo-border-solid)] px-3.5 py-2.5 text-sm text-[color:var(--ibo-ink)] focus:outline-none focus:border-[rgba(254, 157, 85,0.55)]">
            ...
            <div className="space-y-1.5">
              ...
              <textarea rows={3} value={disputeEvidence} onChange={(e) => setDisputeEvidence(e.target.value)}
                ...
                className="w-full rounded-xl bg-transparent border border-[color:var(--ibo-border-solid)] px-3.5 py-2.5 text-sm text-[color:var(--ibo-ink)] placeholder:text-[color:var(--ibo-muted)] resize-none focus:outline-none focus:border-[rgba(254, 157, 85,0.55)]"
              />
...
function SideRow({ label, value, mono, accent, valueClass }) {
  return (
    <div className="flex items-center justify-between gap-4">
      ...
      <span className={`text-xs font-semibold text-right truncate ${mono ? 'font-mono' : ''} ${valueClass || (accent ? 'text-[#FE9D55]' : 'text-white/80')}`}>
        {value}
      </span>
    </div>
  );
...
function CountdownTimer({ deadline }) {
  ...
  if (!left) return null;
  return (
    <div className={`flex items-center gap-1.5 text-xs font-mono font-bold mt-1 ${left === 'Expired' ? 'text-red-400' : 'text-[#FE9D55]'}`}>
      <Clock size={11} />{left}
    </div>
  );
}
