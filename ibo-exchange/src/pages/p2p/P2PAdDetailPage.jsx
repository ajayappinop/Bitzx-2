...
function StatCard({ label, value, accent }) {
  return (
    <div className="rounded-xl p-3.5" style={{ background: 'transparent', border: '1px solid var(--ibo-border-solid)' }}>
      <p className="text-[10px] font-bold uppercase tracking-widest text-white/35 mb-1">{label}</p>
      <p className={`text-sm font-extrabold tabular-nums ${accent ? 'text-[#FE9D55]' : 'text-white'}`}>{value}</p>
    </div>
  );
}
  ...
  return (
    <div className="ibo-page font-ui">
      <div className="w-full px-4 sm:px-5 md:px-6 lg:px-8 xl:px-10 2xl:px-12 py-6 sm:py-8 pb-16">
        <Link to="/p2p" className="inline-flex items-center gap-1.5 text-white/50 hover:text-[#FE9D55] text-sm mb-6 transition-colors">
        ...
        <div className="grid lg:grid-cols-[1fr,380px] gap-6">
          ...
          <div className="space-y-4">
            ...
            {/* Maker card */}
            <div className="ibo-hover-lift ibo-hover-glow rounded-xl border border-[color:var(--ibo-border-solid)] bg-transparent p-6">
              <div className="flex items-center gap-4 mb-5">
                <div className="w-14 h-14 rounded-full flex items-center justify-center text-xl font-extrabold shrink-0"
                  style={{ background: 'rgba(254, 108, 2,0.15)', color: '#FE9D55', border: '2px solid rgba(254, 108, 2,0.25)' }}>
                ...
                <div>
                  <div className="flex items-center gap-2 text-white font-bold text-lg">
                    ...
                    {ad.maker?.is_merchant && (
                      <span className="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-extrabold uppercase text-[#FE9D55]"
            ...
            {/* Payment methods */}
            <div className="rounded-xl border border-[color:var(--ibo-border-solid)] bg-transparent p-5">
              ...
              <div className="flex flex-wrap gap-2">
                {(ad.payment_methods || []).map((p) => (
                  <span key={p.pm_id} className="px-3 py-1.5 rounded-xl text-xs font-bold text-[#FE9D55]"
                    style={{ background: 'rgba(254, 108, 2,0.1)', border: '1px solid rgba(254, 108, 2,0.22)' }}>
                    {p.type}
                  </span>
                ))}
              </div>
            ...
            <div className="flex items-start gap-3 rounded-2xl p-4"
              style={{ background: 'rgba(254, 108, 2,0.06)', border: '1px solid rgba(254, 108, 2,0.18)' }}>
              <Clock size={15} className="text-[#FE9D55] shrink-0 mt-0.5" />
              <p className="text-sm text-white/75 leading-relaxed">
                Payment window: <strong className="text-[#FE9D55]">{ad.payment_window_min} minutes</strong>.
          ...
          <div className="h-fit rounded-2xl p-6 space-y-5"
            ...
            <div className="absolute -top-px left-6 right-6 h-px rounded-full"
              style={{ background: 'linear-gradient(90deg,transparent,rgba(254, 108, 2,0.5),transparent)' }} />
            ...
            <div>
              ...
              <input type="number" value={fiatAmount} onChange={(e) => setFiatAmount(e.target.value)}
                ...
                style={{ background: 'transparent', border: '1px solid var(--ibo-border-solid)', outline: 'none' }}
                onFocus={(e) => { e.target.style.borderColor = 'rgba(254, 108, 2,0.55)'; e.target.style.boxShadow = '0 0 0 3px rgba(254, 108, 2,0.1)'; }}
            ...
            {fiatAmount && (
              <div className="flex items-center justify-between rounded-xl px-4 py-3"
                style={{ background: 'rgba(254, 108, 2,0.07)', border: '1px solid rgba(254, 108, 2,0.15)' }}>
                <span className="text-sm text-white/60">You will {isBuyerView ? 'receive' : 'send'}</span>
                <span className="text-[#FE9D55] font-extrabold tabular-nums">{cryptoEst} {ad.asset}</span>
              </div>
            )}
            ...
            <div>
              ...
              {isBuyerView ? (
                ...
                <div className="flex items-start gap-2.5 rounded-xl p-3.5"
                  style={{ background: 'rgba(0, 168, 118,0.07)', border: '1px solid rgba(0, 168, 118,0.2)' }}>
                  <AlertCircle size={13} className="text-gold shrink-0 mt-0.5" />
                  <span className="text-sm text-white/70">No matching payment method.{' '}
                    <Link to="/p2p/payment-methods" className="text-[#FE9D55] font-semibold hover:underline">Add one</Link> first.
                  </span>
                </div>
              )}
            </div>
            ...
            <button onClick={openOrder} disabled={busy || !canOrder}
              ...
              style={!user || !canOrder ? {
                background: 'transparent', border: '1px solid var(--ibo-border-solid)', color: 'var(--ibo-muted)',
              } : isBuyerView ? {
