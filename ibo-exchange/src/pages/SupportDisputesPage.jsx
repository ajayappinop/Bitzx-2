...
export default function SupportDisputesPage({ accountMode = false } = {}) {
  const { user } = useAuth();
  ...
  return (
    <div className={accountMode ? 'font-ui min-w-0' : 'ibo-page font-ui'}>
      <div className={accountMode ? 'w-full min-w-0 space-y-5' : 'w-full max-w-[1300px] mx-auto px-4 sm:px-6 py-8 space-y-6'}>
        {!accountMode ? (
        ...
        <p className="text-[13px] text-[color:var(--ibo-ink-secondary)] m-0">
          {user?.email || 'Logged in user'} · Open tickets: {openCount}
        </p>
        )}
