/**
 * Landing — INR deposit & INR withdrawal for Indian users (Delta India rails).
 */
      ...
      <div className="relative ibo-landing-container ibo-section-y">
        ...
        >
          ...
          <h2 className="ibo-title-lg mb-4">
            Margin &amp; P/L in <span className="text-gradient">Indian Rupees</span>
          </h2>
          <p className="ibo-lead text-zinc-400 max-w-none">
            Conveniently deposit INR from your bank, trade crypto F&amp;O without holding underlying coins, and
            withdraw INR to your linked bank or UPI — with clear status in your wallet history.
          </p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-2 gap-5 lg:gap-6 max-w-5xl mx-auto">
          ...
          >
            ...
            <ul className="relative space-y-2.5 mb-6">
              {['Bank transfer or UPI', 'UTR + payment proof in wallet', 'Trade capital after review'].map((t) => (
          ...
          >
            ...
            <p
              ...
              style={{ color: 'var(--ibo-ink-secondary)' }}
            >
              Request an INR payout to your verified bank or UPI profile and track approval with payout references
              in wallet history — cash out without complex crypto conversions.
            </p>

            <ul className="relative space-y-2.5 mb-6">
              {['Verified bank / UPI payout profile', 'INR credited after review', 'Email updates on status'].map((t) => (
                <li key={t} className="flex items-start gap-2.5 text-sm" style={{ color: 'var(--ibo-ink)' }}>
            ...
            >
              Withdraw INR <ArrowRight size={16} />
            </Link>
          </motion.article>
        </div>
