/**
 * Delta Exchange–inspired auth chrome (IBO theme).
 * Used by LoginPage / RegisterPage / ForgotPassword / ResetPassword.
 * Classic IBO login/register layouts are parked as *.classic.jsx — do not delete.
 */
  ...
  return (
    <Link to="/" className={`inline-flex items-center ${className}`}>
      <img
        src={LOGO}
        alt="IBO Exchange"
        className="ibo-brand-logo h-10 sm:h-11 w-auto max-w-[220px]"
...
export function AuthField({
  ...
  return (
    <div>
      {label ? (
        <label className="block text-[13px] font-semibold text-[color:var(--ibo-ink-secondary)] mb-2">{label}</label>
      ) : null}
      <div
        className={`ibo-auth-field flex items-center h-12 rounded-xl border px-3.5 transition-colors ${
          error
            ? 'border-red-500/50 bg-red-500/[0.04]'
            : 'border-[color:var(--ibo-border-solid)] bg-[color:var(--ibo-elevated)] focus-within:border-[#0ea4ab]/50'
        }`}
      >
        {Icon ? <Icon size={16} className="text-[color:var(--ibo-muted)] mr-2.5 shrink-0" /> : null}
        <input
          type={type}
          value={value}
          onChange={onChange}
          ...
          className="ibo-auth-input flex-1 min-w-0 bg-transparent text-[15px] text-[color:var(--ibo-ink)] outline-none placeholder:text-[color:var(--ibo-muted)] disabled:opacity-60 caret-[#C5E35B]"
        />
        {rightSlot}
      </div>
      {error ? (
