...
function TokenAvatar({ item, size = 28 }) {
  ...
  return (
    <div
      className="rounded-full bg-[rgba(91,184,255,0.2)] flex items-center justify-center text-[#5BB8FF] font-bold shrink-0"
  ...
  return (
    <div ref={wrapRef} className="relative">
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-[11px] font-bold border transition-colors ${
                depositOnlyFilter === f.id
                  ? 'border-[rgba(91,184,255,0.5)] bg-[rgba(91,184,255,0.15)] text-[#5BB8FF]'
      ...
      <button
        ...
        disabled={disabled}
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between gap-2 bg-surface-card border border-surface-border rounded-xl px-4 py-3 focus:border-[rgba(91,184,255,0.5)] transition-colors disabled:opacity-50"
      >
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            >
              {error ? (
                <p className="px-4 py-3 text-xs text-[#5BB8FF]">{error}</p>
              ) : null}
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 px-4 py-2.5 hover:bg-surface-hover transition-colors disabled:opacity-40 ${
                      active ? 'bg-[rgba(91,184,255,0.1)]' : ''
                    }`}
                  >
                    <TokenAvatar item={it} size={24} />
                    <div className="flex-1 min-w-0 text-left">
                      <p className={`text-sm font-semibold truncate ${active ? 'text-[#5BB8FF]' : 'text-white'}`}>
                        {it.asset}
                        {it.is_listed ? (
                        ...
                        {!canDeposit ? (
                          <span className="ml-1.5 text-[9px] font-bold uppercase text-[#5BB8FF]/90">Soon</span>
                        ) : null}
                      </p>
                      <p className="text-[11px] text-zinc-400 truncate">
                        {it.token_name}
                    ...
                    {active ? <Check size={14} className="text-[#0EA4AB] shrink-0" /> : null}
                  </button>
                );
              })}
              ...
              {hasMore && !loadingMore && visibleCount >= grouped.length && (
                <button
                  type="button"
                  onClick={onLoadMore}
                  className="w-full py-2.5 text-xs font-bold text-[#5BB8FF] hover:bg-[rgba(91,184,255,0.1)] border-t border-surface-border"
                >
