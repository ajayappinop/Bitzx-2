    ...
    return (
      <button
        ...
        className={`w-full text-left rounded-xl border px-4 py-3 transition-colors ${
          selected
            ? 'border-[rgba(91,184,255,0.5)] bg-[rgba(91,184,255,0.1)]'
            : isPlanned
              ? 'border-white/8 bg-white/[0.02] opacity-60 cursor-not-allowed'
              : 'border-surface-border bg-surface-card/40 hover:border-[rgba(91,184,255,0.3)] hover:bg-surface-card/70'
        }`}
      >
        <div className="flex items-start gap-3">
          <span
            className={`mt-1 w-4 h-4 rounded-full border-2 shrink-0 flex items-center justify-center ${
              selected ? 'border-[#5BB8FF]' : 'border-zinc-500'
            }`}
          >
            {selected ? <span className="w-2 h-2 rounded-full bg-[#5BB8FF]" /> : null}
          </span>
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold text-white">
              {n.label || n.network}
              {n.testnet ? (
                <span className="ml-2 text-[10px] font-bold uppercase text-[#5BB8FF]/90">
                  testnet
                </span>
              ) : null}
            </p>
