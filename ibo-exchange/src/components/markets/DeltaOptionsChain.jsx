...
export default function DeltaOptionsChain({
  ...
  const SideCells = ({ q, side, itm }) => {
        ...
        <>
          {Array.from({ length: 7 }).map((_, i) => (
            <td
              ...
              style={{ color: 'var(--ibo-muted)', borderColor: 'var(--ibo-border-solid)', background: itm ? 'rgba(254, 108, 2,0.04)' : undefined }}
            >
              —
            </td>
          ))}
    ...
    const bg = itm ? 'rgba(254, 108, 2,0.06)' : undefined;
                  ...
                  <tr key={row.strike} className="hover:bg-[color:var(--ibo-hover)]">
                    ...
                    <td
                      ...
                      style={{
                        ...
                        background: nearAtm ? 'rgba(254, 108, 2,0.08)' : 'var(--ibo-elevated)',
                      }}
                    >
                      <Link
                        to={`${tradeBasePath}/${encodeURIComponent(underlying)}`}
