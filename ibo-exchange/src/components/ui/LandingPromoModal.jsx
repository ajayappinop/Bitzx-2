...
const CTA_CLASS =
  'group inline-flex w-full sm:w-fit items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold text-[#050a1a] shadow-lg transition-all font-ui';
const CTA_STYLE = {
  background: 'linear-gradient(135deg, #0EA4AB, #5BB8FF)',
  boxShadow: '0 8px 24px rgba(14,164,171,0.28)',
};

function PromoBrand({ label = 'IBO', centered = true }) {
  return (
    <div className={`flex items-center gap-2 font-ui ${centered ? 'justify-center' : ''}`}>
      <div className="relative shrink-0">
        <div
          ...
          style={{ background: 'rgba(91,184,255,0.28)' }}
        />
    ...
    return (
      <div className={`relative flex items-center justify-center w-full ${visualMin} px-3 shrink-0`}>
        <div
          ...
          style={{ background: 'radial-gradient(ellipse 85% 70% at 50% 100%, rgba(91,184,255,0.16), transparent 70%)' }}
        />
        <motion.img
          src={img}
          alt=""
          ...
          >
            ...
            <div className="p-1 sm:p-1.5 space-y-0.5 sm:space-y-1">
              <div
                ...
                style={{
                  background: 'linear-gradient(135deg, rgba(14,164,171,0.12), rgba(91,184,255,0.1))',
                  borderColor: 'rgba(14,164,171,0.25)',
                }}
              />
      ...
      <div className="flex flex-col sm:grid sm:grid-cols-[1.05fr_0.95fr]">
        <div className="relative z-[2] flex flex-col px-4 sm:pl-7 sm:pr-4 pt-12 sm:pt-12 pb-2 sm:pb-4 text-left">
          <div className="mb-3 sm:mb-5">
            <PromoBrand label={data.brand_label || 'IBO'} centered={false} />
          </div>
          ...
          <p className="text-[#0EA4AB] font-bold text-xs sm:text-sm mb-4 leading-snug">{data.event_line}</p>
        ...
        <div className="relative flex items-end justify-center sm:justify-end sm:pr-2 px-2 pb-3 sm:pb-4 shrink-0">
          <CoinVisual imageUrl={data.image_url} brandLabel={data.brand_label || 'IBO'} compact />
        </div>
      </div>
    </div>
  );
        ...
        <div className="flex flex-wrap items-center justify-center gap-x-2 gap-y-1 text-xs sm:text-sm font-bold">
          {features.map((f, i) => (
            <span key={f} className="inline-flex items-center gap-2">
              {i > 0 ? <span className="text-[#0EA4AB]/50 font-normal">|</span> : null}
      ...
      <div className="promo-download-bar px-4 sm:px-6 py-3 sm:py-4 text-center shrink-0">
        {available ? (
          isGooglePlay ? (
            ...
            >
              <span className="inline-flex items-center justify-center gap-2 text-[color:var(--ibo-ink)] font-semibold text-sm sm:text-base">
                <Download size={17} className="text-[#0EA4AB] shrink-0" />
                <span className="underline underline-offset-4 decoration-[#0EA4AB]/50">
          ...
          <p className="inline-flex items-center justify-center gap-2 text-[color:var(--ibo-muted)] text-sm">
            ...
            <Sparkles size={13} className="text-[#5BB8FF] shrink-0" />
                ...
                <div className="flex justify-center gap-2 py-2.5 sm:py-3">
                  {slides.map((s, i) => (
                    <button
                      key={s.key}
                      type="button"
                      aria-label={s.type === 'coin' ? 'IBO coin slide' : 'Mobile app slide'}
                      ...
                      className={`rounded-full transition-all duration-300 ${
                        i === index
                          ? 'w-7 h-2 shadow-[0_0_12px_rgba(91,184,255,0.45)]'
                          : 'w-2 h-2'
                      }`}
                      style={
                        i === index
                          ? { background: 'linear-gradient(90deg, #0EA4AB, #5BB8FF)' }
