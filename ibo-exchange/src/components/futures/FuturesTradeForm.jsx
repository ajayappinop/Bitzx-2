...
export default function FuturesTradeForm({ symbol, limitPriceSeed = null }) {
  ...
  const submit = async () => {
    ...
    if (insufficientIboFee) {
      setErr(`Insufficient IBO for fee (need ~${formatIboFee(estFeeIbo)}).`);
      return;
    }
    try {
      setBusy(true);
      ...
      <div className="flex items-center gap-0 px-3 border-b border-[color:var(--ibo-border)]">
        {TYPES.map((t) => {
          ...
          return (
            ...
            >
              {t.label}
              {on ? <span className="absolute left-2 right-2 bottom-0 h-0.5 rounded-full bg-[#C5E35B]" /> : null}
            </button>
          );
        })}
      </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide px-3 py-3 space-y-3">
        {isMarket ? (
          <div className="flex items-center justify-between rounded-md bg-[color:var(--ibo-elevated)] border border-white/[.06] px-3 py-2">
            ...
            <LivePriceValue value={summaryPx > 0 ? summaryPx : 0} tick={tick} fallback={pricePlaceholder || '—'} cls="text-[13px] text-[#C5E35B] font-bold" />
          </div>
        ) : (
          <div>
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] text-[color:var(--ibo-muted)]">Price</span>
              <button type="button" onClick={snapToLatest} className="text-[11px] font-semibold text-[#C5E35B] hover:underline">
                Latest
              </button>
            </div>
            <input
              ...
              className="w-full h-10 bg-[color:var(--ibo-card)] border border-[color:var(--ibo-border-solid)] rounded-md px-3 text-[13px] font-mono text-white focus:outline-none focus:border-[#C5E35B]/50 placeholder:text-white/30"
            />
            {type === 'maker' && limitCrossBook ? (
              <p className="text-[10px] mt-1 text-rose-400">Maker Only cannot cross the book — adjust price.</p>
            ) : null}
            {limitRole && type !== 'maker' ? (
              <p className={`text-[10px] mt-1 ${limitRole === 'maker' ? 'text-emerald-400' : 'text-[#C5E35B]'}`}>
        ...
        <div className="grid grid-cols-5 gap-1">
          {SIZE_PCTS.map((p) => (
            <button
              ...
              className="text-[11px] py-1.5 rounded border border-[color:var(--ibo-border)] text-[color:var(--ibo-muted)] hover:border-[#C5E35B]/40 hover:text-[#C5E35B] disabled:opacity-35 font-semibold"
            >
              {p}%
            </button>
          ))}
        ...
        {kycBlocked ? (
          <div className={`rounded-lg p-3 border ${
            kyc?.status === 'pending' ? 'bg-[#C5E35B]/10 border-[#C5E35B]/25' : 'bg-rose-500/10 border-rose-500/25'
          }`}>
            ...
            <p className="text-[11px] text-white/70 mb-2 leading-relaxed">
              {kyc?.status === 'pending'
                ...
                : 'Complete KYC to place futures orders on IBO Exchange.'}
            </p>
            <Link to="/kyc" className="text-[12px] font-bold text-[#C5E35B] hover:underline">
              {kyc?.status === 'pending' ? 'Check status' : 'Get verified →'}
            </Link>
          </div>
        ) : null}
        ...
        {kycBlocked ? (
          <Link
            to="/kyc"
            className="w-full h-11 rounded-md text-[14px] font-extrabold bg-[#C5E35B] text-[#0a0f1a] hover:brightness-110 flex items-center justify-center"
          >
            Get Verified To Trade
          </Link>
        ) : (
        ...
        <label className="flex items-center gap-2 text-[11px] text-[color:var(--ibo-muted)] cursor-pointer">
          <input type="checkbox" checked={reduceOnly} onChange={(e) => setRO(e.target.checked)} className="accent-[#C5E35B]" />
          Reduce Only
        </label>
      </div>
    </div>
...
const LiveMarketTicker = memo(function LiveMarketTicker({
  ...
  return (
    <div className="rounded-lg bg-[color:var(--ibo-elevated)] border border-white/[0.06] px-3 py-2 grid grid-cols-2 gap-y-1 text-[11px]">
      <PriceCell label="Mark"     value={mark}    tick={tick} cls="text-[#5BB8FF]" />
