...
export function validateStrongPassword(raw) {
  ...
  // Enforce strong passwords by default. Set VITE_AUTH_RELAXED=1 only for local dev.
  if (import.meta.env.VITE_AUTH_RELAXED === '1' || import.meta.env.VITE_AUTH_RELAXED === 'true') {
    return null;
  }
...
export function getPasswordStrengthMeta(password) {
  ...
  const colors = ['', '#ef4444', '#FE6C02', '#60a5fa', '#22c55e'];
