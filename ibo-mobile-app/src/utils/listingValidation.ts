...
function normUrl(raw: string | undefined, required: boolean): { ok: boolean; msg?: string; value?: string } {
  const s = (raw || '').trim();
  if (!s) {
    return required ? { ok: false, msg: 'URL is required' } : { ok: true, value: '' };
  }
...
function validateContract(network: string, address: string): string | null {
...
export function validateListingForm(form: ListingFormFields): string | null {
...
export const DEFAULT_LISTING_NETWORKS = [
  'ERC-20 (Ethereum)',
  'BEP-20 (BNB Chain)',
  'TRC-20 (Tron)',
  'Bitcoin Network',
